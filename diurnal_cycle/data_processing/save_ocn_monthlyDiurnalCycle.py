...
def main(year, month):
    ...
    if xy_method in ['points', 'grid']:
        t1 = datetime.datetime.now()
        ds = get_mon_di_cy(year, month, 
                           lats, lons, z_lim,
                           xy_method, interp_method)
    ...
    out_fn = 'ocn_' + "{:0>4d}".format(year) + '_' + "{:0>2d}".format(month) + '_meanDiurnalCycle.nc'
...
def get_mon_di_cy(yr, mn, lat_list, lon_list, max_depth,
