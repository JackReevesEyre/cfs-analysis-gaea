...
def main(year, month, xy_method, var_type):
    ...
    elif var_type == 'surface_temp':
        var_list = ['temp', 'xu_ocean', 'yu_ocean']
    elif var_type == '3D':
        var_list = ['temp', 'salt', 'u', 'v']    
    elif var_type == 'all_vars':
        var_list = ['temp', 'salt', 'u', 'v',
                    'eta_t', 'sfc_hflux', 'pme', 'pme_net', 'pme_restore',
                    'evap', 'fprec', 'lprec', 
                    'swflx', 'lw_heat', 'evap_heat', 'sens_heat',
                    'total_ocean_evap', 'tau_x', 'tau_y', 'wt',
                    'mld', 'hblt', 'diff_cbt_kpp_t', 'diff_cbt_kpp_s']
    else:
        sys.exit('var_type not recognized, should be in {surface, 3D, all_vars}')
    ...
    out_fn = 'ocn_' + "{:0>4d}".format(year) + '_' + "{:0>2d}".format(month) \
        + '_' + xy_method + '_' + var_type + '_hourly.nc'
...
def get_hourly_points(yr, mn, lat_list, lon_list, max_depth,
                      interp_how, var_list):
...
if __name__ == '__main__':
    ...
    xy = cla[1]
    vt = cla[2]
    for yr in np.arange(2005, 2006): #np.arange(2002, 2006):
        for mn in np.arange(3,13): #np.arange(1,13):
            main(yr, mn, xy, vt) 
