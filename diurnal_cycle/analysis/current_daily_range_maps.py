...
Usage:
    $ python current_daily_range_maps.py plot_date plot_var
where:
    plot_date is formatted like yyyy_mm_dd 
...
def main(plot_date, plot_var):
    ...
    if plot_var in ['u', 'v']: 
        ...
        ds = ds.isel(st_ocean=0).max(dim='time', keep_attrs=True) - \
            ds.isel(st_ocean=0).min(dim='time', keep_attrs=True) 
        ...
        print(ds)
        plot_title = plot_dets(plot_var, 'plotname') + ' diurnal range at ' + \
    ...
    plotfn = 'current_daily_range_map_' + \
